...
def _dispatch_action(session: Session, *, token, actor_id: uuid.UUID) -> dict:
    ...
    if action == "constraint:approve":
        ...
        if swap_svc.has_required_manager_row(
            session, request_id=resource_id, side="requester", commander_id=actor_id
        ):
            swap_svc.approve_manager_row(
                session, request_id=resource_id, side="requester", commander_id=actor_id, actor_id=actor_id
            )
            return {"action": action, "status": "ok"}
        ...
        swap_svc.approve_manager_side_override(session, request_id=resource_id, side="requester", actor_id=actor_id)
        return {"action": action, "status": "ok"}
