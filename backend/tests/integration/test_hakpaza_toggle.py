...
def test_hakpaza_routes_403_by_default(client: TestClient, admin_session: Session):
...
def test_hakpaza_routes_200_when_explicitly_enabled(client: TestClient, admin_session: Session):
    admin = create_soldier(admin_session, personal_number=f"hk3_{_uid()}", role="admin")
    set_setting(admin_session, "forced_callup.enabled", True, actor_id=admin.id)
    admin_session.commit()
