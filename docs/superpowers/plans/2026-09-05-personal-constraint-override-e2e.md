...
**Goal:** Add real-browser E2E coverage proving the personal-constraint-override feature (already merged to `dev`) behaves correctly end to end: a duty manager can manually assign a soldier who has an approved personal constraint to a *range*, given an override reason, while the CP-SAT algorithm's auto-assign path always hard-excludes that same soldier from *duties* — and document the real (asymmetric) behavior of range auto-select (soft conflict, not hard exclusion), plus two discovered duty-side product gaps: neither of the two UI paths to a duty assignment currently allows overriding a constraint at all (one has no override UI; the other's override UI has no reachable trigger).
...
- [ ] **~~Step 4/5~~ RESCOPED — see ruling below.** Original Steps 4/5 asked for a test proving manual override succeeds (with a reason) via `ShiftAssignModal`'s "Replace" flow, which requires a weapon-ineligible original assignee as a precondition. **This precondition is not reachable through the current live UI + seed data at all**, confirmed by direct investigation (not guessed):
  - Every non-officer weapon-tier duty type (`שמירות`, `אבט"ש`) also carries a separate, unrelated `requires_mitvahim` eligibility gate. No seeded soldier has `last_mitvahim_date` set, so **100% of candidates are hard-blocked** for these duty types regardless of weapon status — confirmed empirically (120/120 candidates blocked in the manual-assignment modal).
  - The remaining weapon-tier duty types are officer-only, but `constrainedSoldier` (needed as the replacement) is a non-officer — structurally excluded there too.
  - The one theoretical CP-SAT-based bypass (disabling weapon-qualification enforcement on an algorithm run) requires a toggle that only exists on `AlgorithmRunForm.tsx`, which is dead code behind the `/algorithm` route that `App.tsx` unconditionally redirects away from. The actually-reachable `AlgorithmInlinePanel.tsx` has no such toggle.
  - The only remaining path (grant provisional eligibility via a future range assignment, then cancel it to flip `weapon_ineligible` back to true after the fact) is a materially larger, multi-system fixture (Ranges UI + attendance/cancellation + finding the actual recheck trigger) that was not pursued, per the plan's own constraint against forcing untestable/synthetic-to-the-point-of-fragile paths.

  **Ruling:** this documents a second real product gap alongside Step 6's — not just "the bulk modal has no override UI," but "the modal that DOES have override UI (`ShiftAssignModal`) has no reachable trigger for the precondition it's built to handle," meaning duty-side manual constraint override is presently unreachable by any real user through either path. Replace Steps 4/5 with:

- [ ] **Step 4 (rescoped): Test — document that no live UI path currently produces a weapon-ineligible primary assignee for a non-officer weapon-tier duty**

  Attempt the most plausible real path (assign any non-officer to a fresh `שמירות` shift via the standard manual-assignment modal) and assert every candidate row is blocked with the mitvahim-gate text (`services/eligibility.py`'s `duty_type_ineligibility_reason()` message — read the exact current string), proving the precondition itself — not just the override UI — is unreachable. This is the duty-side analogue of Step 6: a documented gap, not a defect in the test. File a background task noting both duty-side gaps together (the bulk modal's missing override UI, and the Replace flow's unreachable trigger) so a human can decide whether `ShiftAssignModal`'s override logic should be exposed somewhere reachable (e.g. directly in the bulk modal, closing both gaps at once).

- [ ] **Step 5 (rescoped): Skip — no separate reason-required negative case exists without a reachable positive case to negate.** Do not write a test for this; note its absence in the seam-inventory comment instead of leaving a silently-missing case.
