...
## Task 13 (follow-up, not implemented in this branch): enlistment-time `next_rank_date` initialization

**Status:** Identified by the final whole-branch review after Tasks 1-12 shipped. Deliberately deferred to its own follow-up branch/session rather than folded into a fix commit — it's a real scope gap (missed by all 12 original tasks, not a bug in any one of them) and deserves its own TDD cycle.

**The gap:** the spec's "Initial `next_rank_date`" section says: *"Set at enlistment: when a soldier is created/enrolled with an initial rank, `next_rank_date = enlistment_date + <that rank's configured months_to_next>`, with `next_rank_date_overridden = false`"* and *"`current_rank_since` must be set everywhere `rank` is written: the enlistment-time initializer (= `enlistment_date`) and the promotion worker."* No task implemented this. The only writers of these columns are `update_soldier_profile` (Task 10) and the worker (Task 9) — neither fires at enlistment/enrollment/import time.

**Practical effect:** a newly registered/approved/imported soldier never enters the advancement pipeline. They're partially rescued by `recompute_affected_soldiers`'s fallback (`s.current_rank_since or s.enlistment_date`), so an admin re-saving the interval table backfills everyone retroactively — but a soldier who joins *after* that save gets nothing until someone manually edits their rank.

**Scope for the follow-up task:**
1. Find every path that creates a soldier with an initial `rank` set (registration/enrollment-approval flow, `onboard_soldier`, and the CSV import paths in `backend/app/services/import_sessions.py:1387-1388,1434-1435` — the latter currently set `next_rank_date` directly without setting `next_rank_date_overridden = True`, which means an imported date gets silently clobbered by the next interval-config change, contradicting the override contract).
2. At each such path, set `current_rank_since = enlistment_date` (or `date.today()` if no `enlistment_date` is known at that point — confirm which) and `next_rank_date = compute_next_rank_date(session, rank=soldier.rank, since=current_rank_since)`, `next_rank_date_overridden = False`. For the CSV import paths specifically, decide whether an imported `next_rank_date` should be treated as an override (`next_rank_date_overridden = True`, since it's an explicit externally-supplied value) — this is the more spec-consistent reading and should be confirmed with the human before implementing.
3. Add tests: a soldier created via the real enrollment-approval path gets a correctly computed `next_rank_date`/`current_rank_since` with no interval-table re-save needed; an imported soldier with an explicit `next_rank_date` is marked overridden and survives a subsequent interval-config change unchanged.

**Not required to reproduce the reviewer's investigation from scratch** — the file:line references above are already resolved; start there.
