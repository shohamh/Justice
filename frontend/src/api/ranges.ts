...
export interface FoodSpecialConstraint { soldier_id:string; soldier_name:string; food_type:string; constraint:string; }
export interface FoodAssignmentSummary { counts:Record<string, number>; special_constraints:FoodSpecialConstraint[]; }
export interface FoodSummary { primary:FoodAssignmentSummary; reserve:FoodAssignmentSummary; }
...
export function getMyRanges(soldierId:string,dateFrom?:string,dateTo?:string):Promise<RangeEvent[]>{const params=new URLSearchParams({soldier_id:soldierId});if(dateFrom)params.set("date_from",dateFrom);if(dateTo)params.set("date_to",dateTo);return api.get(`/ranges?${params.toString()}`).then(r=>r.data);}
...
export interface RangeCandidate { soldier_id:string; full_name:string; personal_number:string; reason_code:string; explanation:string; conflict_warning:string|null; personal_constraint_conflict:boolean; auto_selectable?:boolean; }
