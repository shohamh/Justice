...
vi.mock("../auth/AuthContext", () => ({
  useAuth: () => ({
    user: { id: "commander-1", is_commander: true, role: "commander" },
  }),
}));
...
vi.mock("../components/UpcomingSnapshot", () => ({
  default: () => <div data-testid="upcoming-snapshot" />,
}));
...
vi.mock("../components/DutyPotentialPanel", () => ({
  default: () => <div data-testid="duty-potential" />,
}));
vi.mock("../components/dashboard/PendingApprovalsWidget", () => ({
  default: () => <div data-testid="pending-approvals" />,
}));
vi.mock("../components/EntriesExitsPanel", () => ({
  default: () => <div data-testid="entries-exits-panel" />,
}));
vi.mock("../components/UnitCalendar", () => ({
  default: () => <div data-testid="unit-calendar" />,
}));
vi.mock("../components/HierarchyTree", () => ({
  default: () => <div data-testid="hierarchy-tree" />,
}));

vi.mock("../api/commanderDashboard", () => ({
  getSummary: vi.fn().mockResolvedValue({}),
  getDashboardSoldiers: vi.fn().mockResolvedValue([]),
  getFairnessInternal: vi.fn().mockResolvedValue(null),
  getFairnessExternal: vi.fn().mockResolvedValue(null),
  getPotential: vi.fn().mockResolvedValue(null),
  getUpcoming: vi.fn().mockResolvedValue(null),
  getAlerts: vi.fn().mockResolvedValue([]),
}));
vi.mock("../api/hierarchy", () => ({
  fetchFullTree: vi.fn().mockResolvedValue([{ id: "node-1", name: "פלוגה א", commander_id: "commander-1" }]),
}));
vi.mock("../api/soldiers", () => ({
  listSoldiers: vi.fn().mockResolvedValue([]),
  getPendingFieldUpdateCount: vi.fn().mockResolvedValue(0),
}));
vi.mock("../api/potential", () => ({
  getPotential: vi.fn().mockResolvedValue({ raw_eligible_count: 0, modifiers: [], final_potential: 0 }),
}));
vi.mock("../api/enrollment", () => ({
  listPendingEnrollments: vi.fn().mockResolvedValue([]),
}));
vi.mock("../api/swaps", () => ({
...
vi.mock("../api/exemptions", () => ({
  getPendingExemptionCount: vi.fn().mockResolvedValue(0),
}));
...
describe("CommandDashboardPage", () => {
  it("fetches commander-scoped ineligible soldiers and keeps other panels available when that query fails", async () => {
