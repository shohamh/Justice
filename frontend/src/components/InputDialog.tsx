...
export default function InputDialog({ open, title, message, label, initialValue = "", placeholder, multiline = false, confirmLabel, cancelLabel, required = false, confirmDisabled = false, onConfirm, onClose }: InputDialogProps) {
    ...
    <EventDetailModal open={open} title={title} onClose={handleClose}>
      <div className="space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <button type="button" data-testid="input-dialog-confirm" disabled={confirmDisabled || (required && !value.trim())} onClick={() => void handleConfirm()} className="rounded bg-indigo-600 px-4 py-2 text-sm text-white hover:bg-indigo-700 disabled:opacity-40">{confirmText}</button>
        </div>
      </div>
    </EventDetailModal>
  );
