...
export default function RangeEditAssignmentsModal({ open, event, soldiers, canManage, onClose, onChanged }: RangeEditAssignmentsModalProps) {
  ...
  async function add(soldier: SoldierDTO) {
    ...
    try {
      ...
      setError(text("ranges.errors.add_assignment", "הוספת השיבוץ נכשלה"));
    } finally {
      setAdding(null);
    }
  }
  ...
  async function autoAssign() {
    ...
    try {
      ...
      setError(text("ranges.errors.auto_assign", "השיבוץ האוטומטי נכשל"));
    } finally {
  ...
  async function confirmDraft(assignmentId: string) {
    ...
    try {
      ...
      setError(text("ranges.errors.confirm_assignment", "אישור השיבוץ נכשל"));
    } finally {
      setConfirming(null);
    }
  }
  ...
  async function confirmAll() {
    ...
    try {
      ...
      setError(text("ranges.errors.confirm_assignments", "אישור השיבוצים נכשל"));
    } finally {
  ...
  const renderAssignment = (a: RangeAssignment) => (
    <li key={a.id} className="flex flex-wrap items-center justify-between gap-2 border-t px-3 py-2 text-sm first:border-t-0 dark:border-gray-600">
      <span>{name(a.soldier_id)} {a.is_draft && <span data-testid={`draft-badge-${a.id}`} className="mr-2 rounded bg-indigo-100 px-1.5 py-0.5 text-xs text-indigo-700">{text("ranges.draft", "טרם נשמר")}</span>}<span className="mr-2 text-xs text-gray-600 dark:text-gray-300">{text("ranges.assignment_reason_label", "סיבת השיבוץ")}: {reasonFor(a)}</span></span>
      {editable && <span className="flex gap-1">
        ...
        {a.is_draft && <button type="button" data-testid={`confirm-draft-${a.id}`} disabled={confirming !== null || confirmingAll} onClick={() => void confirmDraft(a.id)} className={`${actionClass} border-green-600 bg-green-600 text-white`}>{text("ranges.confirm", "אשר")}</button>}
  ...
  return <EventDetailModal open={open} title={text("ranges.edit_assignments", "עריכת שיבוצים")} subtitle={`${event.location} · ${event.date}`} onClose={onClose}>
    <div className="space-y-4">
      ...
      {shortfall !== null && <div className="rounded bg-amber-100 px-3 py-2 text-sm">{text("ranges.auto_assign_shortfall", "לא נמצאו מספיק מועמדים — חסרים {{count}} שיבוצים").replace("{{count}}", String(shortfall))}</div>}
      ...
      {editable && <section className="space-y-2 rounded border p-3 dark:border-gray-600">
        <div className="flex flex-wrap gap-2"><button type="button" data-testid="range-auto-assign" disabled={autoAssigning || (primaryFull && reserveFull)} onClick={() => void autoAssign()} className={`${actionClass} border-blue-600 bg-blue-600 text-white`}>{text("ranges.auto_assign", "שיבוץ אוטומטי")}</button>{assignments.some(a => a.is_draft) && <button type="button" data-testid="range-confirm-all" disabled={confirming !== null || confirmingAll} onClick={() => void confirmAll()} className={`${actionClass} border-green-600 bg-green-600 text-white`}>{text("ranges.confirm_all", "אשר הכל")}</button>}</div>
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" data-testid="range-reserve-toggle" checked={reserve} onChange={e => setReserve(e.target.checked)} /> {text("ranges.assign_as_reserve", "שיבוץ כרזרבה")}</label>
        <input data-testid="range-soldier-search" value={query} onChange={e => setQuery(e.target.value)} placeholder={text("ranges.search_soldier", "חיפוש חייל")} className="w-full rounded border p-2 text-sm dark:bg-gray-700" />
        <div className="max-h-40 overflow-y-auto">{candidates.map(s => <button key={s.id} type="button" data-testid={`add-soldier-${s.id}`} disabled={adding !== null || (reserve ? reserveFull : primaryFull)} onClick={() => void add(s)} className="block w-full border-t p-2 text-right text-sm hover:bg-gray-50 disabled:opacity-50 dark:border-gray-600 dark:hover:bg-gray-700">{adding === s.id ? text("ranges.adding", "מוסיף...") : `${s.full_name} (${s.personal_number})`}</button>)}{candidates.length === 0 && <p className="p-2 text-sm text-gray-500">{text("ranges.no_available_soldiers", "אין חיילים זמינים")}</p>}</div>
      </section>}
      <div className="flex justify-end border-t pt-3 dark:border-gray-600"><button type="button" onClick={onClose} className="rounded border px-3 py-1.5 text-sm dark:border-gray-600">{text("ranges.close", "סגור")}</button></div>
    </div>
  </EventDetailModal>;
}
