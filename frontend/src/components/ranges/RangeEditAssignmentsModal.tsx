  ...
  return <EventDetailModal open={open} title={text("ranges.edit_assignments", "עריכת שיבוצים")} subtitle={`${event.location} · ${event.date}`} onClose={onClose}>
    <div className="space-y-4">
      ...
      {editable && <section className="space-y-2 rounded border p-3 dark:border-gray-600">
        ...
        <input data-testid="range-soldier-search" value={query} onChange={e => setQuery(e.target.value)} placeholder={text("ranges.search_soldier", "חיפוש חייל")} className="w-full rounded border p-2 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100" />
