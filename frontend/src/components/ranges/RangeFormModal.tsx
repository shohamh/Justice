...
export default function RangeFormModal({ open, event, hierarchyNodeId, onClose, onSubmit }: Props) {
  ...
  return <EventDetailModal open={open} title={event ? "עריכת מטווח" : "מטווח חדש"} onClose={onClose}>
    <form onSubmit={submit} className="space-y-3" data-testid={event ? "range-form" : "create-event-form"}>
      <label className="block text-sm">סוג<select data-testid={event ? "edit-range-type" : "new-range-type"} value={form.range_type} onChange={e=>set("range_type",e.target.value)} className="mt-1 w-full rounded border p-2"><option value="laser">{RANGE_TYPE_LABELS.laser}</option><option value="live">{RANGE_TYPE_LABELS.live}</option><option value="alal">{RANGE_TYPE_LABELS.alal}</option></select></label>
