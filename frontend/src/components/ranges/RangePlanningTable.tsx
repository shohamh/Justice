...
export default function RangePlanningTable({rows,onRowClick,rowActions,filters,sort}:Props){ const columns:PlanningColumn<RangeEvent>[]=[{key:"date",label:"תאריך",sortValue:e=>e.date,render:e=><span dir="ltr">{e.date}</span>},{key:"type",label:"סוג",render:e=>RANGE_TYPE_LABELS[e.range_type] ?? e.range_type},{key:"location",label:"מיקום",render:e=><button type="button" onClick={()=>onRowClick(e)} className="text-indigo-600 hover:underline">{e.location}</button>},{key:"primary",label:"ראשיים",render:e=>`${e.primary_filled}/${e.required_count}`},{key:"reserve",label:"רזרבה",render:e=>`${e.reserve_filled}/${e.reserve_count}`},{key:"status",label:"סטטוס",render:e=>RANGE_EVENT_STATUS_LABELS[e.status] ?? e.status}]; return <PlanningTable columns={columns} rows={rows} getRowId={e=>e.id} getRowLabel={e=>e.location} onRowClick={onRowClick} rowActions={rowActions} filters={filters} sort={sort} />; }
