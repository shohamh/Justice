...
export default function AddRootNodeDialog({ onClose, onCreated }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-xl p-6 w-96" onClick={(e) => e.stopPropagation()} data-testid="add-root-dialog">
        <h3 className="font-semibold mb-4">{t("team.add_root_node")}</h3>
        <form onSubmit={onSubmit} className="space-y-3">
          <select className="border rounded p-1 w-full" value={level} onChange={(e) => setLevel(e.target.value)} data-testid="root-level">
            {LEVEL_ORDER.map((l) => (
              <option key={l} value={l}>{t(`team.level_${l}`)}</option>
            ))}
          </select>
          <input className="border rounded p-1 w-full" value={name} onChange={(e) => setName(e.target.value)} placeholder={t("team.node_name")} required data-testid="root-name" />
